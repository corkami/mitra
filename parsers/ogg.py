...
def crc32ogg(d):
	val = 0
	for s in d:
		val = ((val << 8) ^ LOOKUP[s ^ (val >> 24)]) & 0xffffffff
	return val
...
class parser(FType):
	...
	def wrap(self, data, id=b"junk"):
		data_s = len(data)
		segcount = 1 + data_s // 256
		header = b"".join([
			b"OggS",   # Magic
			b"\0",     # Version
			b"\2",     # Type = first
			b"\0"*8,   # Granule Position
			b"JUNK",   # Serial Number
			b"\0" * 4, # Sequence Number
			b"\0" * 4, # CRC (set to 0 for computation)
			# Segment count
			bytes([segcount]),
			# Segment Table
			bytes([0xff] * (segcount - 1) + [data_s % 256]),
			])

		# CRC computation with null placeholder, page included
		wrapped = header + data
		crc = crc32ogg(wrapped)
		wrapped = wrapped[:0x16] + int4l(crc) + wrapped[0x16+4:]
